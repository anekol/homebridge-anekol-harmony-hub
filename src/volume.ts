...
export class AnekolHarmonyHubVolumeHelper {
	...
	constructor(
		...
		service.getCharacteristic(this.hap.Characteristic.On)
			.on('get', this.getVolumeOn.bind(this, this.hub.slug))
	...
	// get volume on
	private async getVolumeOn(hub_slug: string, callback: CharacteristicGetCallback) {
		const data = await this.status(hub_slug)
		if (data.off) {
			if (this.verboseLog)
				this.log.info('Get Volume On: not allowed - hub is off');
	...
	private async getVolume(hub_slug: string, callback: CharacteristicGetCallback) {
		...
		if (data.off) {
			if (this.verboseLog)
				this.log.info('Get Volume: not allowed - hub is off');
			callback(HAPStatus.NOT_ALLOWED_IN_CURRENT_STATE)
		} else {
			const vol = DEFAULT_VOLUME
			if (this.verboseLog)
				this.log.info('Get Volume: ' + vol);
			callback(NO_ERRORS, vol);
		}
	}
